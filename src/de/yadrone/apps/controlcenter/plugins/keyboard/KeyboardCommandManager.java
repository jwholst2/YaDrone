...
//		System.out.println("Key released: " + e.getKeyChar());
		...
		if (key != KeyEvent.VK_A && key!=KeyEvent.VK_Y) {
			drone.hover();
		}
	}
...
//		}
		...
		switch (key)
		{
			case KeyEvent.VK_A:
				if (thr == null) {
					// start LSL control
					runnable = new LSLdata(drone);
					thr = new Thread(runnable);
					thr.start();
				}
				else {
					System.out.println("New LSL thread NOT CREATED. End old thread first.");
				}
				break;
			case KeyEvent.VK_Y:
				// end LSL control
				if (thr != null) {
					...
					catch(InterruptedException e) { System.out.println("thr.join() interrupted exception!!"); }
				}
				break;
